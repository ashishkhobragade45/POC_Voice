...
import com.example.poc_voice.BuildConfig;
import com.example.poc_voice.utils.Constants;
import com.example.poc_voice.fcm.IncomingCallNotificationService;
import com.example.poc_voice.R;
import com.example.poc_voice.fcm.ShowNotificationIntentService;
import com.example.poc_voice.utils.SoundPoolManager;
import com.example.poc_voice.model.M_Token;
import com.example.poc_voice.model.M_main;
import com.example.poc_voice.network.RetrofitClient;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // accessToken = getResources().getString(R.string.auttoken);
        getToken();
       ...
       btn_auth.setOnClickListener(new View.OnClickListener() {
           ...
           public void onClick(View view) {
               Intent intent = new Intent(MainActivity.this, AuthClass.class);
    ...
    private void getToken() {
        ...
        jsonObject.addProperty("username", "jeremy.mercer@401auto.ca");
        jsonObject.addProperty( "password", "Test@Test123");
        jsonObject.addProperty(  "grantType", "password");
        jsonObject.addProperty( "deviceId", "fqwtTes_vvqnmsoa0IZI3G:APA91bGzrXP8R2viE5RwJ0TimU4wOmIG5CvWLOwrH7L7FsNB_dTGCa_Jb0j0m2jpNCigaDJBWokxtONzbAg1_1oFo2jh6cB27-PpTnDKRgyivO5Op5s-BvA_r4A49dQLPJ81OmoqtySC");
        jsonObject.addProperty("deviceType", "WEB");

        RetrofitClient.getInstance().getMyApi().gettokennew(jsonObject).enqueue(new Callback<M_main>() {
            @Override
            public void onResponse(retrofit2.Call<M_main> call, Response<M_main> response) {
                String newToken = response.body().getMessage().getAccessToken();
                getNewToken(newToken);
            }

            @Override
            public void onFailure(retrofit2.Call<M_main> call, Throwable t) {

            }
        });

       /* RetrofitClient.getInstance().getMyApi().gettoken("abc").enqueue(new Callback<String>() {
            @Override
            public void onResponse(retrofit2.Call<String> call, Response<String> response) {
                if (response.isSuccessful()) {
                    Gson gson = new Gson();
                    String successResponse = gson.toJson(response.body());
                    Log.e("LOG_TAG", "successResponse: " + response.body());
                    Log.e("LOG_TAG", "INsuccessResponse: " );
            ...
            public void onFailure(retrofit2.Call<String> call, Throwable t) {
                Log.e("LOG_TAG", "outsuccessResponse: ");
            }
        });
*/
    }

    private void getNewToken(String newToken) {

        RetrofitClient.getInstance().getMyApi().gettokennewsub("Bearer "+newToken).enqueue(new Callback<M_Token>() {
            @Override
            public void onResponse(retrofit2.Call<M_Token> call, Response<M_Token> response) {
                accessToken = response.body().getMessage();
                voiceSetUp();
            }

            @Override
            public void onFailure(retrofit2.Call<M_Token> call, Throwable t) {
    ...
    private void resetUI() {
        callActionFab.show();
        muteActionFab.setImageDrawable(ContextCompat.getDrawable(MainActivity.this, R.drawable.ic_mic_white_24dp));
        holdActionFab.hide();
    ...
    protected void onResume() {
        ...
        registerReceiver();
        startAudioSwitch();
        ShowNotificationIntentService.startActionHide(getApplicationContext());
    ...
    private View.OnClickListener callActionFabClickListener() {
        return v -> {
            alertDialog = createCallDialog(callClickListener(), cancelCallClickListener(), MainActivity.this);
            alertDialog.show();
        };
    }
    ...
    private View.OnClickListener hangupActionFabClickListener() {
        return v -> {
            SoundPoolManager.getInstance(MainActivity.this).playDisconnect();
            resetUI();
            disconnect();
